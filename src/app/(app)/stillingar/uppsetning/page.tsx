  ...
  return (
    <div className="px-4 py-4 pb-24">
      <div className="mb-4 flex items-center gap-3">
        ...
        <h1 className="font-semibold">Uppsetning {new Date().getFullYear()}</h1>
      </div>

      <section className="mb-6">
        <p className="mb-3 text-sm font-medium">Snúningsröð:</p>
        <RotationSorter households={households} order={order} onChange={setOrder} />
      </section>

      <section className="mb-6">
        <p className="mb-2 text-sm font-medium">Sameiginleg vika (vor):</p>
        <select
          value={springWeek ?? ''}
          onChange={(e) =>
            setSpringWeekState(e.target.value ? Number(e.target.value) : null)
          }
      ...
      {preview.length > 0 && (
        <section className="mb-6">
          <p className="mb-2 text-sm font-medium">Forskoðun:</p>
          <div className="max-h-64 overflow-y-auto rounded border border-gray-200">
            {preview.map((a) => {
              ...
              return (
                <div
                  key={a.week_number}
                  className="flex items-center gap-2 border-b border-gray-100 px-3 py-1.5 text-xs last:border-0"
                >
                  <span className="w-8 text-gray-400">V.{a.week_number}</span>
                  {isShared ? (
